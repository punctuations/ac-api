...
There are no `headers` required for this API, simply fetch `https://ac-api.vercel.app/v1/` and you're good to go!

Example:

```js highlight=2
const getSong = () => {
  fetch("https://ac-api.vercel.app/v1/")
    .then((res) => res.json())
    .then(parseResponse)
    .then(setResponse);
};
...
```jsx highlight=2
useEffect(() => {
  fetch("https://ac-api.vercel.app/v1/").then((res) => res.json());
}, []);
```
